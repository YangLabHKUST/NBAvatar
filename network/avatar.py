...
class AvatarNet(nn.Module):
    def __init__(self, opt):
        ...
        self.front_back_view_sep = opt.get('front_back_view_sep', False)
        ...
        if self.with_viewdirs:
            ...
            if self.front_back_view_sep:
                self.front_viewdir_net = nn.Sequential(
                    nn.Conv2d(1, 64, 4, 2, 1),
                    nn.LeakyReLU(0.2, inplace = True),
                    nn.Conv2d(64, 128, 4, 2, 1)
                )
                self.back_viewdir_net = nn.Sequential(
                    nn.Conv2d(1, 64, 4, 2, 1),
                    nn.LeakyReLU(0.2, inplace = True),
                    nn.Conv2d(64, 128, 4, 2, 1)
                )
    ...
    def get_viewdir_feat(self, items):
        ...
        if self.front_back_view_sep:
            front_viewdirs = self.opt.get('weight_viewdirs', 1.) * self.front_viewdir_net(front_viewdirs)
            back_viewdirs = self.opt.get('weight_viewdirs', 1.) * self.back_viewdir_net(back_viewdirs)
        else:
