...
class DualStyleUNet(nn.Module):
    def __init__(self, inp_size, inp_ch, out_ch, out_size, style_dim, n_mlp, middle_size=8, c_dim=0,
        ...
        for layer_idx in range(self.num_layers):
            # Calculate the resolution based on the layer index
            # Each pair of layers (2 convs) increases resolution by 2x
            res = self.middle_log_size + (layer_idx // 2) + 1
...
class StyleUNet(nn.Module):
    def __init__(self, inp_size, inp_ch, out_ch, out_size, style_dim, n_mlp, middle_size=8, c_dim=0,
                 channel_multiplier=2, blur_kernel=[1, 3, 3, 1], lr_mlp=0.01, use_wt=True, 
                 return_feature=False, return_feature_size=256):
        super().__init__()
        ...
        self.return_feature_size = return_feature_size
        self.middle_log_size = int(math.log(middle_size, 2))
        ...
        self.feature_dim = self.channels[self.return_feature_size]

        self.log_size = int(math.log(out_size, 2)) - 1 if self.use_wt else int(math.log(out_size, 2))
        ...
        for i in range(self.middle_log_size + 1, self.log_size + 1):  # 4, 5, 6, 7, 8, 9
            ...
            if use_wt:
                ...
                self.to_rgbs.append(ToRGB(in_channel=out_channel, style_dim=style_dim, out_channel=out_ch))

            in_channel = out_channel
        if use_wt:
            self.iwt = InverseHaarTransform(out_ch)
        else:
            self.iwt = nn.Identity()
    ...
    def forward(
        ...
        for from_rgb, cond_conv in zip(self.from_rgbs, self.cond_convs):
            cond_img, cond_out = from_rgb(cond_img, cond_out)
            cond_out = cond_conv(cond_out)
            if self.return_feature and cond_out.shape[-1] == self.return_feature_size:
                out_feature = cond_out
            cond_list.append(cond_out)
            cond_num += 1
        ...
        for conv1, conv2, noise1, noise2, to_rgb in zip(
            ...
            out = conv2(out, latent[:, i + 1], noise=noise2)
            skip = to_rgb(out, latent[:, i + 2], skip)
            if view_feature is not None and view_feature.shape[1] == out.shape[1] and i % 2 == 0:
                view_feature = F.interpolate(view_feature, out.shape[-2:], mode = 'bilinear')  # (B, 128, 256. 256)
                out = out + view_feature
                add_view = True
            i += 2
        if view_feature is not None:
            assert add_view, 'view_feature is not None but not added'
        image = self.iwt(skip)
        if return_latents:
            return image, latent
        elif self.return_feature:
            assert out_feature is not None, 'out_feature is None but return_feature is True'
            return image, out_feature
        else:
            return image, None
