...
class MLPLinear(nn.Module):
    def __init__(self,
        ...
        if self.last_std is not None:
            self.fc_list[-1].weight.data.normal_(0, self.last_std)
            if self.fc_list[-1].bias is not None:
                self.fc_list[-1].bias.data.zero_()
