...
def export_pts(pts: torch.Tensor, color: torch.Tensor = None, normal: torch.Tensor = None, scalars: dict = {}, filename: str = "default.ply", skip_color: bool = False, **kwargs):
    from pandas import DataFrame
    from pyntcloud import PyntCloud

    data = dict()
    pts = pts.cpu().numpy()  # always blocking?
    pts = pts.reshape(-1, 3)
...
def save_mesh_as_ply(path, vertices, faces = None, normals = None, colors = None, radius = None):
    import struct
    fp = open(path, 'w')
    fp.write('ply\n')
    ...
    if radius is not None:
        fp.write('property float radius\n')
    # if faces is not None:
    ...
    for i in range(vertices.shape[0]):
        ...
        r = radius[i] if radius is not None else None
        if normals is None:
            if colors is None:
                if radius is None:
                    data = struct.pack('3f', v[0], v[1], v[2])
                else:
                    data = struct.pack('3f1f', v[0], v[1], v[2], r[0])
            else:
                if radius is None:
                    data = struct.pack('3f3B', v[0], v[1], v[2], c[0], c[1], c[2])
                else:
                    data = struct.pack('3f3B1f', v[0], v[1], v[2], c[0], c[1], c[2], r[0])
        else:
            if colors is None:
                if radius is None:
                    data = struct.pack('6f', v[0], v[1], v[2], n[0], n[1], n[2])
                else:
                    data = struct.pack('6f1f', v[0], v[1], v[2], n[0], n[1], n[2], r[0])
            else:
                if radius is None:
                    data = struct.pack('6f3B', v[0], v[1], v[2], n[0], n[1], n[2], c[0], c[1], c[2])
                else:
                    data = struct.pack('6f3B1f', v[0], v[1], v[2], n[0], n[1], n[2], c[0], c[1], c[2], r[0])
        fp.write(data)
    if faces is not None:
