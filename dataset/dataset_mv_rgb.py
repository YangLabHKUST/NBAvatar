...
class MvRgbDatasetBase(Dataset):
    ...
    def __init__(
        ...
        mode = '3dgs',
        pos_map_mode = 'raw',
        boundary_mask_kernel_size = 5,
        filter_repreated_frames = False,
        **kwargs
    ):
        ...
        self.pos_map_mode = pos_map_mode
        self.mode = mode  # '3dgs' or 'nerf'
        self.boundary_mask_kernel_size = boundary_mask_kernel_size
        self.load_cam_data()
        self.load_smpl_data()
        ...
        if self.training and filter_repreated_frames:
            new_pose_list = [self.pose_list[0]]
            for i in range(1, len(self.pose_list)):
                pose0 = self.smpl_data['body_pose'][new_pose_list[-1]][3:22*3]
                pose1 = self.smpl_data['body_pose'][self.pose_list[i]][3:22*3]
                if np.abs(pose0 - pose1).mean() > 1.5e-2:
                    new_pose_list.append(self.pose_list[i])
    ...
    def getitem(self, index, training = True, **kwargs):
        ...
        if training:
            ...
            boundary_mask_img, mask_img = self.get_boundary_mask(mask_img, self.boundary_mask_kernel_size)
...
class MvRgbDatasetActorsHQ(MvRgbDatasetBase):
    ...
    def filter_missing_files(self):
        import json
        if os.path.exists(self.data_dir + '/4x/valid_set.json'):
            print(f'Filtering missing files from {self.data_dir}/4x/valid_set.json')
            with open(self.data_dir + '/4x/valid_set.json', 'r') as f:
                valid_set = json.load(f)
            valid_set = [int(x) for x in valid_set]
            self.data_list = [data for data in self.data_list if data[0] in valid_set]
