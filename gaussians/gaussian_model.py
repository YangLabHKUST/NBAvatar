...
class GaussianModelTorch(nn.Module):
    def __init__(self, points, colors, sh_degree : int = 0, spatial_lr_scale : float = 1.0, init_occ = 0.05, opt_xyz_offset = False):
        super(GaussianModelTorch, self).__init__()
        
        self.active_sh_degree = 0
        self.max_sh_degree = sh_degree  
        self.spatial_lr_scale = spatial_lr_scale
        self.opt_xyz_offset = opt_xyz_offset
        ...
        # dist2 = torch.clamp_min(distCUDA2(fused_point_cloud), 0.0000001)
        dist2 = torch.clamp_min(knn_points(fused_point_cloud[None], fused_point_cloud[None], K = 4)[0][0, :, 1:].mean(-1), 0.0000001)
        scales = torch.log(torch.clamp(torch.sqrt(dist2), max=0.005))[...,None].repeat(1, 3)
        rots = torch.zeros((fused_point_cloud.shape[0], 4), device="cuda")
        rots[:, 0] = 1

        opacities = torch.logit(init_occ * torch.ones((fused_point_cloud.shape[0], 1), dtype=torch.float, device="cuda"))

        if self.opt_xyz_offset:
            print('Opt xyz offset, set _xyz grad to False')
            self._xyz = nn.Parameter(fused_point_cloud, requires_grad=False)
            self._offset = nn.Parameter(torch.zeros_like(fused_point_cloud), requires_grad=True)
        else:
    ...
    def get_xyz(self):
        if self.opt_xyz_offset:
            return self._xyz + torch.tanh(self._offset) * 0.008 # a trick
        else:
            return self._xyz
